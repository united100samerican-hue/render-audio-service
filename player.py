...
class VoicePlayer:
    ...
    def _meta_from_info(self, info, fallback_url=''):
        ...
        return {'video_id': video_id, 'webpage_url': webpage_url, 'title': title, 'duration': duration, 'thumbnail': thumbnail, 'is_live': is_live, 'direct_url': direct_url}
    ...
    async def _probe_url(self, source_id: str):
        ...
        def _do():
            opts = self._yt_opts(str(TMP / f'probe_{uuid.uuid4().hex}.%(ext)s'), {'youtube': {'player_client': ['mweb', 'web_safari', 'android'], 'formats': ['missing_pot']}, 'youtubepot-bgutilhttp': {'base_url': [self.pot_provider_url]}})
    ...
    async def meta(self, chat_id, source_type, source_id, title='', duration=0):
        ...
        if source_type in {'voice', 'audio', 'document', 'file_id'} or self._looks_like_file_id(source_id):
            return {'ok': True, 'state': {'source_type': source_type or 'file_id', 'source_id': source_id, 'title': title, 'duration': duration, 'video_id': '', 'webpage_url': '', 'thumbnail': '', 'is_live': False, 'direct_url': ''}}
...
async def _download_url(self, url: str, chat_id: str):
        ...
        attempts = [
            {'extractor_args': {'youtube': {'player_client': ['mweb', 'web_safari', 'android'], 'formats': ['missing_pot']}, 'youtubepot-bgutilhttp': {'base_url': [self.pot_provider_url]}}, 'format': 'bestaudio[protocol^=m3u8]/bestaudio[ext=m4a]/bestaudio[ext=webm]/bestaudio/best'},
            {'extractor_args': {'youtube': {'player_client': ['web_safari', 'mweb', 'android'], 'formats': ['missing_pot']}, 'youtubepot-bgutilhttp': {'base_url': [self.pot_provider_url]}}, 'format': 'bestaudio[ext=m4a]/bestaudio[ext=webm]/bestaudio/best'},
        ]
        last_error = None
        last_meta = {}
    ...
    async def _direct_play_url(self, direct_url: str, chat_id: str):
        if not direct_url:
            return False
        try:
            await self._invoke('play', int(chat_id), direct_url)
            return True
        except TypeError:
            await self._invoke('play', chat_id=int(chat_id), media=direct_url)
            return True
        except Exception as e:
    ...
    async def start(self, chat_id, source_type, source_id, title='', duration=0):
        async with self.lock:
            ...
            if source_type == 'file_id' or self._looks_like_file_id(source_id):
                ...
                self.state[chat_id] = {'source_type': 'file_id', 'source_id': source_id, 'title': resolved_title, 'duration': resolved_duration, 'path': source_path, 'status': 'playing', 'position': 0, 'webpage_url': '', 'thumbnail': '', 'video_id': '', 'direct_url': '', 'is_live': False}
                last_error = None
                for _ in range(2):
                    try:
            ...
            if self._is_url(source_id):
                ...
                resolved_title = str(title or probe.get('title') or '')
                resolved_duration = int(duration or probe.get('duration') or 0)
                direct_url = str(probe.get('direct_url') or '').strip()
                if direct_url:
                    self.state[chat_id] = {'source_type': source_type or 'url', 'source_id': source_id, 'title': resolved_title, 'duration': resolved_duration, 'path': direct_url, 'status': 'playing', 'position': 0, 'webpage_url': str(probe.get('webpage_url') or source_id), 'thumbnail': str(probe.get('thumbnail') or ''), 'video_id': str(probe.get('video_id') or ''), 'direct_url': direct_url, 'is_live': bool(probe.get('is_live', False))}
                    ok = await self._direct_play_url(direct_url, chat_id)
                    if ok:
                        bg = asyncio.create_task(self._background_download(chat_id, source_id))
                        self.background_tasks[chat_id] = bg
                        return self.state[chat_id]
                source_path, source_meta = await self._download_url(source_id, chat_id)
                self.state[chat_id] = {'source_type': source_type or 'url', 'source_id': source_id, 'title': resolved_title or source_meta.get('title') or '', 'duration': resolved_duration or source_meta.get('duration') or 0, 'path': source_path, 'status': 'playing', 'position': 0, 'webpage_url': str(source_meta.get('webpage_url') or source_id), 'thumbnail': str(source_meta.get('thumbnail') or probe.get('thumbnail') or ''), 'video_id': str(source_meta.get('video_id') or probe.get('video_id') or ''), 'direct_url': str(probe.get('direct_url') or ''), 'is_live': bool(probe.get('is_live', False))}
                last_error = None
                for _ in range(2):
                    try:
                        print('before_play', chat_id, source_path)
            ...
            self.state[chat_id] = {'source_type': stype, 'source_id': sid, 'title': resolved_title, 'duration': resolved_duration, 'path': source_path, 'status': 'playing', 'position': 0, 'webpage_url': source_meta.get('webpage_url', ''), 'thumbnail': source_meta.get('thumbnail', ''), 'video_id': source_meta.get('video_id', ''), 'direct_url': source_meta.get('direct_url', ''), 'is_live': bool(source_meta.get('is_live', False))}
            last_error = None
            for _ in range(2):
                try:
                    print('before_play', chat_id, source_path)
