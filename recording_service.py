        ...
        for method, field_name in (("sendAudio", "audio"), ("sendDocument", "document")):
            try:
                ...
                if r.status_code < 400 and isinstance(j, dict) and j.get("ok"):
                    return True, method
                logger.error("upload failed method=%s status=%s body=%s", method, r.status_code, getattr(r, "text", "")[:800])
            except Exception:
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("recording_service_latest:app", host="0.0.0.0", port=int(os.getenv("PORT", "10000")), reload=False)