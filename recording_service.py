...
def new_output_path(self, chat_id: str) -> Path:
...
logger.error("telegram upload failed endpoint=%s status=%s body=%s", endpoint, r.status_code, getattr(r, "text", ""))
except Exception:
...
session.mode = "raw"
async def start(self, chat_id: str, deliver_to: str, started_by: str = "", group_title: str = "", title: str = "") -> dict[str, Any]:
...
session = RecorderSession(chat_id=chat_id, deliver_to=deliver_to, status="recording", started_at=time.time(), output_path=self._new_output_path(chat_id), started_by=started_by, group_title=group_title, title=title)
...
pass
raise
async def stop(self, chat_id: str, auto: bool = False, deliver_to: str = "", stopped_by: str = "", group_title: str = "", caption: str = "") -> dict[str, Any]:
async with self.lock:
...
self.sessions.pop(chat_id, None)
return {"ok": False, "error": "deliver_to_missing", "file_name": session.output_path.name, "mode": session.mode}
meta = [x for x in [session.group_title, group_title, caption, f"by:{stopped_by}" if stopped_by else ""] if x]
...
data = {"ok": True, "recording": False, "file_name": session.output_path.name, "sent": sent, "send_result": result, "mode": session.mode, "size": size, "last_error": session.last_error}
try:
if sent:
...
return {"ok": True, "recording": session.status == "recording", "mode": session.mode, "file_name": session.output_path.name, "started_at": session.started_at, "last_error": session.last_error, "deliver_to": session.deliver_to}
...
try:
return await manager.start(payload.chat_id, payload.deliver_to, payload.started_by, payload.group_title, payload.title)
except Exception as exc:
...
raise HTTPException(status_code=503, detail="service_not_ready")
try:
return await manager.stop(payload.chat_id, deliver_to=payload.deliver_to, stopped_by=payload.stopped_by, group_title=payload.group_title, caption=payload.caption)
except Exception as exc:
